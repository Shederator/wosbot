...
| **CI — macOS Installers** | Automatically for pull requests that touch packaging-related paths | Builds and smoke-tests Stable and Nightly macOS app-images and `.pkg` artifacts. PR runs upload short-lived pkg artifacts, never a GitHub Release. |
...
| **Release — macOS Stable or Nightly** | Daily at 03:47 UTC (05:47 CEST / 04:47 CET) | Attaches the Apple Silicon `.pkg` to the Windows-published Nightly immutable release and merges `macos-arm64` into the project-signed update manifest. |
| **Release — macOS Stable or Nightly** | Manually from `main` with channel and an already-published immutable version | Attaches the matching Stable or Nightly `.pkg` after Windows has published that tag. |
