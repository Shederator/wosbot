...
- Task Workbench UI starts standalone controlled runs through
  `ControlledTaskExecutionService` and renders its in-memory step trace.
...
and report a step history. Without an attached controlled session, those
boundaries execute directly and do not change scheduler behavior.
