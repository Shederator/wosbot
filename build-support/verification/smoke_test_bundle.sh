...
#
# Structural verification (build-support/verification/verify_bundle.py) proves
# the right files are in the ZIP. It cannot prove they actually link together: a dependency dropped from a
...
#
# Usage: build-support/verification/smoke_test_bundle.sh <bundle.zip>
...
app_jar="$(find . -maxdepth 1 -name 'frostguard-desktop-*.jar' | head -n 1)"
watcher_jar="$(find . -maxdepth 1 -name 'frostguard-watcher-*.jar' | head -n 1)"
...
javac -nowarn -cp "${app_jar}${classpath_separator}lib/*" -d . Probe.java
java -cp "${app_jar}${classpath_separator}lib/*${classpath_separator}." Probe
