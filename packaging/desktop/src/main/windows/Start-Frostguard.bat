...
set "APP_JAR="
set "APP_JAR_COUNT=0"
for %%F in ("frostguard-desktop-*.jar") do if exist "%%~fF" (
    set /a APP_JAR_COUNT+=1
    set "APP_JAR=%%~fF"
)

if %APP_JAR_COUNT% EQU 0 (
    echo [ERROR] No frostguard-desktop-*.jar was found next to this launcher.
    echo Extract the complete desktop bundle before starting Frostguard.
    goto :failed
)
if %APP_JAR_COUNT% GTR 1 (
    echo [ERROR] Multiple frostguard-desktop-*.jar files were found.
