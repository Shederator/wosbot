#!/usr/bin/env python3
"""Self-tests for ci/verify_bundle.py.
...
Run with:  python3 ci/test_verify_bundle.py
"""
...
RUNTIME_JARS = (
    ...
    + [
        ...
        "lib/fg-vision-9.9.9.jar",
        "lib/fg-engine-9.9.9.jar",
    ]
...
def build_bundle(
    *,
    ...
    with zipfile.ZipFile(handle, "w") as bundle:
        ...
        if include_launcher:
            bundle.writestr(
                f"frostguard-{VERSION}.jar", build_launcher_jar(class_path)
            )
    handle.close()
    return handle.name
