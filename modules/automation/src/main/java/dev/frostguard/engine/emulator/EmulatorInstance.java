...
public abstract class EmulatorInstance {
    ...
    // rival cities). The public swipe(..., durationMs) overload above (validating
    // durationMs > 0) gives callers a deliberate, explicit-duration drag instead --
    // routed through this command builder so both overloads share one shell-command
    // format.
